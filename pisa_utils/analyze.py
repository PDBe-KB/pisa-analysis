...
class AnalysePisa:
            ...
            if root and asroot:
                ...
                for interface in interfaces:
                    ...
                    (
                        ...
                        is_ligand,
                    ) = self.get_molecules_dictionary(molecules)
    ...
    def set_results(self, interfaces_results, entry_type, pdb_id, assembly_id):
        ...
        if interfaces_results:
            ...
            assem_dict = {
                "mmsize": interfaces_results.get("assembly_mmsize"),
    ...
    def analyse_pisa_result(
        ...
        """
        interfaces_results = self.process_pisa_interface_xml(
            interfaces_xml_file=interfaces_xml_file, assembly_xml_file=assembly_xml_file
        )
    ...
    def analyize(self):
        pdbid_file = (
            os.path.join(self.input_dir, self.input_cif_file)
            if self.input_cif_file
            else os.path.join(
                self.input_dir,
                "{}-assembly{}.cif.gz".format(self.pdb_id, self.assembly_id),
            )
        )
        ...
        if pdbid_file:
            self.analyse_pisa_result(
                interfaces_xml_file=interfaces_xml_file,
                pdb_id=self.pdb_id,
                assembly_id=self.assembly_id,
                entry_type="assembly",
                assembly_xml_file=assembly_xml_file,
            )
