...
from pisa_to_json.run_pisa import RunPisa
#run_pisalite
#from run_pisa import RunPisa
...
def run():
    ...
    parser.add_argument("-i", "--input_dir", help="input directory with CIF directory/file")
    ...
    parser.add_argument("-o", "--output-dir", help="output directory for JSON and XMLs",required=True)
    ...
    parser.add_argument('--input_cif_file', help='assembly cif file with a seleceted name different from default', type=str)
    parser.add_argument('--force', help='always recalculate with pisa-lite', action='store_true')

    args = parser.parse_args()

    logger = logging.getLogger()
    
    ap = AnalysePisa(pdbid_id=args.pdb_id,assembly_id=args.assembly_id, pisa_config=args.config, output_dir=args.output_dir,force=args.force,result_json_file=args.result_json, input_dir=args.input_dir, input_updated_cif=args.input_updated_cif, input_cif_file=args.input_cif_file)
    ap.run_process()
