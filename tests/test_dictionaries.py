...
class TestDictionaries(TestCase):
    ...
    def test_get_bond_dict(self, mock):
        ...
        # bonds = [bond]
        result = get_bond_dict(bond, "H-bond", "cifpath")
        expected = {
            "bond_distances": [2.99],
    ...
    def test_get_molecules_dict(self):
        ...
        """
        molecule = ET.parse(
            os.path.join(".", "tests", "data", "molecules.xml")
        ).getroot()
        molecules = [molecule]
        expected_dictionary = {
            "molecule_id": "1",
        ...
        self.assertEqual(get_molecules_dict(molecules)[0][0], expected_dictionary)
    ...
    def test_get_molecules_ligand(self):
        ...
        """
        molecule = ET.parse(
            os.path.join(".", "tests", "data", "interfaces_ligand.xml")
        ).getroot()
        molecules = [molecule]
        self.assertTrue(get_molecules_dict(molecules)[2])
