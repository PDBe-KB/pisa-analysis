...
class TestConvertAssemblyXMLToJSON(TestCase):
    ...
    def test_parse_multi_assembly_xml_with_pdb_file(self):
        self.input_xml = self.base_input_dir.joinpath(
            "mock_data", "3hax_assembly_multi_asmset_from_pdb.xml"
        )
        self.expected_json = self.base_input_dir.joinpath(
            "expected_output", "3hax_assembly_multi_asmset_from_pdb.json"
        )

        with tempfile.TemporaryDirectory() as tmp_dir:
            self.output_json = Path(tmp_dir).joinpath(
                "3hax_assembly_multi_asmset_from_pdb.json"
            )
